...
public class MicroControllerLaserDriver extends Thread implements LaserVibrate {
	...
	public void run() {
		while (running) {
			try {
				laserPointer.publish(new DataButton(Laserpointer.class, DataButton.BUTTON_1, (readBL() == 1) ? true
						: false));
				laserPointer.publish(new DataButton(Laserpointer.class, DataButton.BUTTON_2, (readBU() == 1) ? true
						: false));
				laserPointer.publish(new DataButton(Laserpointer.class, DataButton.BUTTON_3, (readBR() == 1) ? true
						: false));
