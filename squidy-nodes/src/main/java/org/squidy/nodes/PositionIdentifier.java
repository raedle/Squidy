 ...
 *         
 * @version $Id: PositionIdentifier.java 772 2011-09-16 15:39:44Z raedle $
 * @since 1.0.0
 */
@XmlType(name = "PositionIdentifier")
@Processor(
	name = "Position Identifier",
	icon = "/org/squidy/nodes/image/48x48/positionidentifier.png",
	description = "/org/squidy/nodes/html/PositionIdentifier.html",
	types = { Processor.Type.FILTER },
	tags = { "position", "movement", "touch", "finger", "path integration", "identification" }
)
public class PositionIdentifier extends AbstractNode {
	
	private int mode = MODE_OFF;
	private int frameRate = 100;
	private int mNoise = 800;
	...
	private int maximumDistanceMultiPoint = 30;
...
//		snapToTicks = true
	...
	private int maximumTimeout = 1000;
...
//		dataPosition2D.setIdentifier(dataPosition2D.getIdentifier());
		...
		if(smoothingSubpixels){
			if(Math.abs(dataPosition2D.getX()-kalmanModels.getLast_x())<=smoothingRangeHorizontal/1000) dataPosition2D.setX(kalmanModels.getLast_x());
			if(Math.abs(dataPosition2D.getY()-kalmanModels.getLast_y())<=smoothingRangeVertical/1000) dataPosition2D.setY(kalmanModels.getLast_y());
		}
