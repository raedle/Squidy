...
public class ConfigManager {
	...
	public ConfigManager(ConfigManagable configManagable, int port) {
		...
		try {
			...
			LOG.fatal("Couldn't load xml configuration file: " + CONFIG_XML);
			return;
		}
	...
	private synchronized void loadConfiguration() throws JAXBException, FileNotFoundException {
		...
		configuration = (Configuration) u.unmarshal(new File(CONFIG_XML));
	}
						...
						if (configConnection.getIdentifier().equals(cam.getId())) {
							...
							if (cam.getMsg().equalsIgnoreCase("start_calibration")) {
								try {
									...
									showWhiteScreen(client, false);
									showPatternScreen(client, true, displayWidth, displayHeight, patternW, patternH);
...
//											displayHeight);
							...
							if (cam.getMsg().equalsIgnoreCase("showWhite")) {
								if (LOG.isDebugEnabled()) {
									LOG.debug("Show white for camera #" + cam.getId());
								}
								showWhiteScreen(client, true);
							}

							if (cam.getMsg().equalsIgnoreCase("start_tracking")) {
								if (LOG.isDebugEnabled()) {
									LOG.debug("Close calibration pattern for camera #" + cam.getId());
								}
								showPatternScreen(client, false);
								showWhiteScreen(client, true);
							}
					...
					if (found) {
						...
						getConfig().getCameraHashtable().get(cam.getId()).setOnline(false);
						return;
					}
				}
			}
	 ...
	 * @param show
	 */
	private void showWhiteScreen(ControlClient client, boolean show) {
		WhiteScreen whiteScreen = new WhiteScreen();
		whiteScreen.setState(show ? Switch.ON : Switch.OFF);
		
		System.out.println("SENDING WHITE SCREEN");
	 ...
	 * @param params
	 */
	private void showPatternScreen(ControlClient client, boolean show, int... params) {
		PatternScreen patternScreen;
		if (params.length == 4) {
			patternScreen = new PatternScreen("\\Display0", params[0], params[1], params[2], params[3]);
		}
		else {
