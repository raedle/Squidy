...
package org.squidy.nodes.laserpointer.configclient.service.image;
...
public class TcpIpJpegImageService extends ImageService {
	private static Logger logger = Logger.getLogger(TcpIpJpegImageService.class);
	...
	public TcpIpJpegImageService(String adr, int port, int w, int h) {
		super(w, h, null);
	...
	private class ImagePollingThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				logger.debug("Expecting: " + size + " bytes.");
				if(debug) {
					debugWriter.newLine();
